...
*/	
	...
	while(tol>umbral && iter<maxiter)
	{
		...
		*/
		...
		MPI_Allgatherv (p1, sizeR, MPI_DOUBLE, aux, sizes, dspls, MPI_DOUBLE, MPI_COMM_WORLD);
		...
		if(sn<=umbral)
		{
			...
			daxpy (&n_dist, &alpha1, p1, &IONE, x1, &IONE);            	// x1=x0+alpha1*p1;
			dcopy (&n_dist, x1, &IONE, x, &IONE);            //x=x1;
		...
		*/
		...
		MPI_Allgatherv (s, sizeR, MPI_DOUBLE, aux, sizes, dspls, MPI_DOUBLE, MPI_COMM_WORLD);
		...
		*/
		...
		//!!!!!!!!!!
		dcopy (&n_dist, x0, &IONE, x1, &IONE);            //x1=x0;
		daxpy (&n_dist, &alpha1, p1, &IONE, x1, &IONE);
		daxpy (&n_dist, &omega1, s, &IONE, x1, &IONE);
