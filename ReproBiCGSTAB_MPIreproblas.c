...
/*
    ...
    if (myId == 0) {
        char name1[50];
        sprintf(name1, "%d.txt", nProcs);
        fp1 = fopen(name1,"w");
        char name2[50];
        sprintf(name2, "%d.txt", nProcs+1);
        fp2 = fopen(name2,"w");
    }
#endif
*/
...
*/
	/*
		...
		if (myId == 0) {
			...
			//tol = exblas::cpu::RoundStickyBit( &h_superacc[0] );
		}
  ...
  double_binned *local_isum = binned_dballoc(K);
  binned_dbsetzero(K, local_isum);
  if(myId == 0)
  {
    isum = binned_dballoc(K);
    binned_dbsetzero(K, isum);
  }
  // Performing local dot
  //binnedBLAS_dbdsum(K, local_n, local_x, 1, local_isum);
  binnedBLAS_dbddot(K, n_dist, res, 1, res, 1, local_isum);
  // Reduce
  MPI_Reduce(local_isum, isum, 1, binnedMPI_DOUBLE_BINNED(K), binnedMPI_DBDBADD(K), 0, MPI_COMM_WORLD);
  if(myId == 0)
  {
    tol = binned_ddbconv(K, isum);
  }
  MPI_Bcast(&tol, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  tol=sqrt(tol);
...
*/	
		...
		if (myId == 0) {
			...
			//rou1 = exblas::cpu::RoundStickyBit( &h_superacc[0] );
		}
  ...
  //binnedBLAS_dbdsum(K, local_n, local_x_shuffled, 1, local_isum);
  binnedBLAS_dbddot(K, n_dist, res0, 1, res00, 1, local_isum);
  MPI_Reduce(local_isum, isum, 1, binnedMPI_DOUBLE_BINNED(K), binnedMPI_DBDBADD(K), 0, MPI_COMM_WORLD);
  if(myId == 0)
  {
    rou1 = binned_ddbconv(K, isum);
  }
  MPI_Bcast(&rou1, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  
	    //dotproduct(res0, res00, &rou1, n_dist, commun); //dot product
		
		//好像是这里res0值一样，但是算出来rou1不同!	
		//rou1是负数,可能算负数有问题!
		//可以把res0和res00输出到文件单独用不同版本内积算一下!
...
//#if 1
    // print aux
    MPI_Allgatherv (res0, n_dist, MPI_DOUBLE, aux, sizes, dspls, MPI_DOUBLE, MPI_COMM_WORLD);
    if (myId == 0) {
        //fprintf(fp, "%d ", iter);
        //fprintf(fp, "%d\n", iter);
        for (int ip = 0; ip < n; ip++)
    ...
    MPI_Allgatherv (res00, n_dist, MPI_DOUBLE, aux, sizes, dspls, MPI_DOUBLE, MPI_COMM_WORLD);
    if (myId == 0) {
        //fprintf(fp, "%d ", iter);
        //fprintf(fp, "%d\n", iter);
        for (int ip = 0; ip < n; ip++)
...
/*
			if(iter==3)
			{
				printf("rou1:%.10e rou0:%.10e alpha0:%.10e omega:%.10e\n",rou1,rou0,alpha0,omega);
			}
*/
...
*/		
	...
	*/	
		...
		if (myId == 0) {
			...
			//rv = exblas::cpu::RoundStickyBit( &h_superacc[0] );
		}
  ...
  //binnedBLAS_dbdsum(K, local_n, local_x_shuffled, 1, local_isum);
  binnedBLAS_dbddot(K, n_dist, res00, 1, v1, 1, local_isum);
  MPI_Reduce(local_isum, isum, 1, binnedMPI_DOUBLE_BINNED(K), binnedMPI_DBDBADD(K), 0, MPI_COMM_WORLD);
  if(myId == 0)
  {
    rv = binned_ddbconv(K, isum);
  }
  MPI_Bcast(&rv, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
			
		//dotproduct(v1, res00, &rv, n_dist, commun); //dot product
		//第3次迭代rv是负数，可能Exblas不能算负数
/*
		if(myId==0)
		{
			printf("iter:%d rv:%.10e\n",iter,rv);
		}
*/
		alpha1=rou1/rv;
		...
		if (myId == 0) 
		{
			...
			//sn = exblas::cpu::RoundStickyBit( &h_superacc[0] );
		}
  ...
  //binnedBLAS_dbdsum(K, local_n, local_x_shuffled, 1, local_isum);
  binnedBLAS_dbddot(K, n_dist, s, 1, s, 1, local_isum);
  MPI_Reduce(local_isum, isum, 1, binnedMPI_DOUBLE_BINNED(K), binnedMPI_DBDBADD(K), 0, MPI_COMM_WORLD);
  if(myId == 0)
  {
    sn = binned_ddbconv(K, isum);
  }
  MPI_Bcast(&sn, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);	
...
*/		
		...
		*/
        ...
        if (myId == 0) 
		{
			...
			//vAux[0] = exblas::cpu::RoundStickyBit( &h_superacc[0] );
            vAux[1] = exblas::cpu::Round( &h_superacc_tol[0] );
			//vAux[1] = exblas::cpu::RoundStickyBit( &h_superacc_tol[0] );
        }
  ...
  //binnedBLAS_dbdsum(K, local_n, local_x_shuffled, 1, local_isum);
  binnedBLAS_dbddot(K, n_dist, t, 1, s, 1, local_isum);
  MPI_Reduce(local_isum, isum, 1, binnedMPI_DOUBLE_BINNED(K), binnedMPI_DBDBADD(K), 0, MPI_COMM_WORLD);
  if(myId == 0)
  {
    ts = binned_ddbconv(K, isum);
  }
  ...
  //binnedBLAS_dbdsum(K, local_n, local_x_shuffled, 1, local_isum);
  binnedBLAS_dbddot(K, n_dist, t, 1, t, 1, local_isum);
  MPI_Reduce(local_isum, isum, 1, binnedMPI_DOUBLE_BINNED(K), binnedMPI_DBDBADD(K), 0, MPI_COMM_WORLD);
  if(myId == 0)
  {
    tt = binned_ddbconv(K, isum);
  }
...
int main (int argc, char **argv) {
    ...
    int k=0;
    int *vptrM = matL.vptr;
    for (i=0; i < matL.dim1; i++) 
	{
	int j;
        for(j=vptrM[i]; j<vptrM[i+1]; j++) 
		{
            sol1L[k] += matL.vval[j];
        }
        k++;
...
*/
    ...
    if (myId == 0) 
	{
		...
		//beta = exblas::cpu::RoundStickyBit( &h_superacc[0] );	
    }
  ...
  // Performing local dot
  //binnedBLAS_dbdsum(K, local_n, local_x, 1, local_isum);
  binnedBLAS_dbddot(K, dimL, sol2L, 1, sol2L, 1, local_isum);
  // Reduce
  MPI_Reduce(local_isum, isum, 1, binnedMPI_DOUBLE_BINNED(K), binnedMPI_DBDBADD(K), 0, MPI_COMM_WORLD);
  if(myId == 0)
  {
    beta = binned_ddbconv(K, isum);
  }
  MPI_Bcast(&beta, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
