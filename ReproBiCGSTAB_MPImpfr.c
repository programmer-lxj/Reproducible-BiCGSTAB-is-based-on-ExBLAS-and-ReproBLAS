...
int mpfrDot(int N, double *a, double *b, double *c, MPI_Comm *commun) 
{
    mpfr_t sum, dot, op;
    mpfr_init2(op, 64);
    //mpfr_init2(dot, 128);
...
*/
	...
	*/
	/*
     ...
     nb = sqrt(nb); //root
	 */
	 mpfrDot(n_dist, res, res, &tol, commun);
	 tol=sqrt(tol);
	 mpfrDot(n_dist, b, b, &nb, commun);
	 nb=sqrt(nb);
...
*/		
		...
		*/
		...
		mpfrDot(n_dist, res0, res00, &rou1, commun);
		if(iter==3)
		{
			printf("rou1:%.10e\n",rou1);
			/*
...
*/
		...
		*/
		...
		mpfrDot(n_dist, v1, res00, &rv, commun);
/*
		if(myId==0)
		{
...
*/
		...
		*/
		mpfrDot(n_dist, t, s, &ts, commun);
		mpfrDot(n_dist, t, t, &tt, commun);
		//omega1 is w1
		omega1 = ts/tt;
		/*
		if(myId==0)
